...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      ...
      obs.subscribe(
        t => subject.onNext(Success(t)),
        e => { subject.onNext(Failure(e)); subject.onCompleted() },
        () => subject.onCompleted())
    ...
    //      val subject = ReplaySubject[Try[S]]
    //
    //      obs.subscribe(
    //        t => {
    //          subject.onNext(requestMethod(t))
    //        },
    //        e => subject.onNext(Failure(e)),
    //        () => subject.onCompleted()
    //      )
    //
    //      subject
